...
async function main() {
  ...
  const post1 = await prisma.post.create({
    data: {
      ...
      content:
        'The Nervous System API models biological pathways: Input-Process-Output cycles, synaptic network transport, and the Blood-Brain Barrier for request validation.',
  ...
  const post2 = await prisma.post.create({
    data: {
      ...
      content:
        'Never trust the client. Syntactic validation verifies standard formats with Zod, while Semantic validation enforces business logic in the service layer.',
      status: PostStatus.PUBLISHED,
    },
  });
  ...
  const post4 = await prisma.post.create({
    data: {
      ...
      content:
        'By treating APIs as sensory organs and motor outputs, we decouple business domain services from raw transport payloads.',
      status: PostStatus.PUBLISHED,
    },
  });
  ...
  const post5 = await prisma.post.create({
    data: {
      ...
      content:
        'Draft study on fast-path short-circuiting in express middleware for early error detection.',
      status: PostStatus.DRAFT,
    },
  });
